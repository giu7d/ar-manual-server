...
export class InitialMigration1603121538418 implements MigrationInterface {
    name = 'InitialMigration1603121538418'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "instruction_source" ("id" uuid NOT NULL, "type" character varying NOT NULL, "src" character varying NOT NULL, "instructionId" uuid NOT NULL, CONSTRAINT "PK_e9ff25cd4d133ff86530dbb690d" PRIMARY KEY ("id"))`);
        ...
        await queryRunner.query(`CREATE TABLE "testbench" ("id" uuid NOT NULL, "componentSerialNumber" character varying NOT NULL, "testBenchSerialNumber" character varying NOT NULL, "isActive" boolean NOT NULL DEFAULT true, "caoId" uuid, CONSTRAINT "REL_cdd09a254cfbbdab841bf7a2c4" UNIQUE ("caoId"), CONSTRAINT "PK_e3db507594e1553fca798c42252" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "warning" ("id" uuid NOT NULL, "description" character varying NOT NULL, "createdAt" TIMESTAMP NOT NULL, "instructionId" uuid NOT NULL, CONSTRAINT "PK_54ddc381cc95ffd6909e427b093" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "instruction" ("id" uuid NOT NULL, "description" character varying NOT NULL, "step" integer NOT NULL, "nextInstructionId" character varying, "testBenchId" uuid NOT NULL, CONSTRAINT "PK_dd8def68dee37e3f878d0f8673a" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "analysis_step" ("id" uuid NOT NULL, "status" character varying NOT NULL, "failure" json NOT NULL, "startedAt" TIMESTAMP NOT NULL, "finishedAt" TIMESTAMP NOT NULL, "analysisId" uuid, "instructionId" uuid, CONSTRAINT "PK_71b3633f0d0dc4582b290101075" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "analysis" ("id" uuid NOT NULL, "status" character varying NOT NULL, "testBenchId" character varying NOT NULL, "startedAt" TIMESTAMP NOT NULL, "finishedAt" TIMESTAMP NOT NULL, "accountId" character varying NOT NULL, CONSTRAINT "PK_300795d51c57ef52911ed65851f" PRIMARY KEY ("id"))`);
